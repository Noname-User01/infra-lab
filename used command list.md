...
## Reverse Proxyの構築
Nginx がFastAPIをを中継する事でNginxが低速なクライアントとの通信を肩代わりし、FastAPI（Uvicorn）を「計算」だけに専念させている

また、ポート集約: 内部では色々なポートで複数のアプリが動いていても、外からは「80番（HTTP）や443番（HTTPS）」だけでアクセス可能になる

「systemd」で管理したことで、「OSのサービス」に昇華させている。
もし仮にアプリがエラーで落ちても、systemdが自動で再起動（Restart=always）してくれる
